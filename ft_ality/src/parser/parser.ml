...
let process_grammar_file path =
  ...
  | ParseError msg ->
    ...
    exit 1
  | ex ->
    print_endline ("Unexpected error: " ^ Printexc.to_string ex);
    exit 1

(** [show_key_mappings key_mappings] prints each key mapping in a human-readable format. *)
let show_key_mappings key_mappings =
  List.iter (fun km ->
    print_endline ("Key: '" ^ km.Key_mappings.key ^ "' -> Action: '" ^ km.Key_mappings.action ^ "'")
  ) key_mappings
...
let show_move_sequences moves =
  List.iter (fun mv ->
    print_endline ("Move Name: '" ^ mv.Moves_parser.name ^ "', Combo: [" ^
                   String.concat ", " mv.Moves_parser.key_combination ^ "]")
  ) moves
